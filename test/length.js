(function () {
    ...
    QUnit.test('with min only', function(assert){
        ...
        const is_min_length = its('hello world').length({min: 5}) ? true : false;
        assert.ok(is_min_length, '"hello world": minimum length is valid');

        const is_not_min_length = its('this a short one').length({min: 20}) ? true : false;
    ...
    QUnit.test('with max only', function(assert){
        ...
        const is_max_length = its('a short text').length({max: 20}) ? true : false;
        assert.ok(is_max_length, '"a short text": maximum length is valid');

        const is_not_max_length = its('this is a very long text').length({max: 20}) ? true : false;
    ...
    QUnit.test('with both min and max', function(assert){
        ...
        const is_min_max_length = its('medium text').length({min: 5, max: 20}) ? true : false;
        assert.ok(is_min_max_length, '"medium text": length is valid');

        const is_not_min_length = its('short text').length({min: 30, max: 50}) ? true : false;
        assert.notOk(is_not_min_length, '"short text": minimum length must be 30 (not valid)');

        const is_not_max_length = its('this is a very very very long text').length({min: 10, max: 18}) ? true : false;
